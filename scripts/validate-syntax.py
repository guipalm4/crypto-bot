    ...
    def validate_pydantic_v2(self, file_path: Path) -> None:
        ...
        try:
            ...
            if "from pydantic import BaseSettings" in content:
                self.errors.append(
                    f"{file_path}: Use 'from pydantic_settings import BaseSettings' instead of 'from pydantic import BaseSettings'"
                )

            if "Field(env=" in content:
                self.errors.append(
                    f"{file_path}: Use 'Field(alias=' instead of 'Field(env=' for Pydantic v2"
                )

            if "class Config:" in content:
                self.errors.append(
                    f"{file_path}: Use 'model_config = SettingsConfigDict()' instead of 'class Config:' for Pydantic v2"
                )

            # Verificar se está usando SettingsConfigDict
            if "BaseSettings" in content and "SettingsConfigDict" not in content:
                self.warnings.append(
                    f"{file_path}: Consider using SettingsConfigDict for better Pydantic v2 compatibility"
                )
    ...
    def validate_sqlalchemy_v2(self, file_path: Path) -> None:
        ...
        try:
            ...
            if "from sqlalchemy.ext.declarative import declarative_base" in content:
                self.errors.append(
                    f"{file_path}: Use 'from sqlalchemy.orm import DeclarativeBase' instead of 'from sqlalchemy.ext.declarative import declarative_base'"
                )

            if "declarative_base()" in content:
                self.errors.append(
                    f"{file_path}: Use 'class Base(DeclarativeBase): pass' instead of 'declarative_base()'"
                )
