...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_trading_pair_base_asset_id'), 'trading_pair', ['base_asset_id'], unique=False)
    op.create_index(op.f('ix_trading_pair_exchange_id'), 'trading_pair', ['exchange_id'], unique=False)
    op.create_index(op.f('ix_trading_pair_quote_asset_id'), 'trading_pair', ['quote_asset_id'], unique=False)
    op.create_index(op.f('ix_trading_pair_symbol'), 'trading_pair', ['symbol'], unique=False)
    ...
    sa.Column('type', sa.Enum('MARKET', 'LIMIT', 'STOP_LOSS', 'STOP_LOSS_LIMIT', 'TAKE_PROFIT', 'TAKE_PROFIT_LIMIT', name='ordertype', native_enum=False, length=50), nullable=False),
    sa.Column('side', sa.Enum('BUY', 'SELL', name='orderside', native_enum=False, length=20), nullable=False),
    sa.Column('status', sa.Enum('PENDING', 'OPEN', 'PARTIALLY_FILLED', 'FILLED', 'CANCELLED', 'EXPIRED', 'REJECTED', 'FAILED', name='orderstatus', native_enum=False, length=50), nullable=False),
    ...
    sa.ForeignKeyConstraint(['trading_pair_id'], ['trading_pair.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_order_created_at'), 'order', ['created_at'], unique=False)
    op.create_index(op.f('ix_order_exchange_id'), 'order', ['exchange_id'], unique=False)
    op.create_index(op.f('ix_order_exchange_order_id'), 'order', ['exchange_order_id'], unique=False)
    ...
    op.create_index(op.f('ix_order_trading_pair_id'), 'order', ['trading_pair_id'], unique=False)
    ...
    sa.Column('side', sa.Enum('LONG', 'SHORT', name='positionside', native_enum=False, length=20), nullable=False),
    sa.Column('status', sa.Enum('OPEN', 'CLOSED', 'LIQUIDATED', name='positionstatus', native_enum=False, length=20), nullable=False),
    sa.Column('quantity', sa.Numeric(precision=20, scale=8), nullable=False),
    ...
    op.create_index(op.f('ix_position_opened_at'), 'position', ['opened_at'], unique=False)
    ...
    op.create_index(op.f('ix_position_trading_pair_id'), 'position', ['trading_pair_id'], unique=False)
    ...
    op.create_index(op.f('ix_trade_exchange_trade_id'), 'trade', ['exchange_trade_id'], unique=False)
...
def downgrade() -> None:
    ...
    sa.UniqueConstraint('name', name=op.f('exchanges_name_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    ...
    sa.Column('base_currency', sa.VARCHAR(length=10), autoincrement=False, nullable=False),
    sa.Column('quote_currency', sa.VARCHAR(length=10), autoincrement=False, nullable=False),
    sa.Column('is_active', sa.BOOLEAN(), server_default=sa.text('true'), autoincrement=False, nullable=True),
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.PrimaryKeyConstraint('id', name=op.f('trading_pairs_pkey')),
    sa.UniqueConstraint('symbol', 'base_currency', 'quote_currency', name=op.f('trading_pairs_symbol_base_currency_quote_currency_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    op.create_index(op.f('idx_trading_pairs_symbol'), 'trading_pairs', ['symbol'], unique=False)
    op.create_index(op.f('idx_trading_pairs_quote_currency'), 'trading_pairs', ['quote_currency'], unique=False)
    op.create_index(op.f('idx_trading_pairs_base_currency'), 'trading_pairs', ['base_currency'], unique=False)
