...
class PluginRegistry(ABC):
    ...
    def load_plugins(self) -> None:
        ...
        try:
            ...
            self._loaded = True
            logger.info(
                f"Loaded {len(self._plugins)} plugins from {self.plugin_directory}"
            )
    ...
    def _validate_plugin(self, plugin_class: type[ABC]) -> None:
        ...
        if inspect.isabstract(plugin_class):
            raise PluginValidationError(
                f"Plugin class {plugin_class.__name__} is abstract"
            )
    ...
    def _get_plugin_name(self, plugin_class: type[ABC]) -> str:
        ...
        if hasattr(plugin_class, "__bases__") and any(
            issubclass(base, ExchangeBase) for base in plugin_class.__bases__
        ):
    ...
    def create_instance(self, name: str, *args: Any, **kwargs: Any) -> ABC:
        """
        ...
        try:
            ...
            raise PluginLoadError(
                f"Failed to create instance of plugin '{name}': {e}"
            ) from e

    def get_instance(self, name: str) -> ABC | None:
        """
...
class ExchangePluginRegistry(PluginRegistry):
    ...
    def _validate_plugin_specific(self, plugin_class: type[ABC]) -> None:
        ...
        if not isinstance(plugin_class, type) or not issubclass(
            plugin_class, ExchangeBase
        ):
            raise PluginValidationError(
                f"Plugin class {getattr(plugin_class, '__name__', str(plugin_class))} is not an ExchangeBase subclass"
            )

        exchange_cls = plugin_class

        # Check if the plugin has required attributes
        required_attributes = ["name", "id", "countries", "urls", "version"]
        for attr in required_attributes:
            if not hasattr(exchange_cls, attr):
                raise PluginValidationError(
                    f"Exchange plugin {exchange_cls.__name__} missing required attribute: {attr}"
                )
        ...
        for method_name in required_methods:
            if not hasattr(exchange_cls, method_name):
                raise PluginValidationError(
                    f"Exchange plugin {exchange_cls.__name__} missing required method: {method_name}"
                )

            method = getattr(exchange_cls, method_name)
            if not callable(method):
                raise PluginValidationError(
                    f"Exchange plugin {exchange_cls.__name__} attribute '{method_name}' is not callable"
                )

    def get_exchange(self, name: str, *args: Any, **kwargs: Any) -> ExchangeBase:
        """
        ...
        return cast(ExchangeBase, instance)
    ...
    def get_exchange_info(self, name: str) -> dict[str, Any]:
        ...
        Raises:
            PluginNotFound: If exchange is not found
        """
        plugin_class = cast(type[ExchangeBase], self.get_plugin(name))
